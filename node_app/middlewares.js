...
console.log('user!!!', data)
 ...
 validateAdminUser: function(req, res, next) {
        if (!(req.headers.authorization && req.headers.authorization.split(' ')[0] === 'JWT')) return next(new Errors.UnAuth("No user token"));
        const data = auth.verifyJwt(req.headers.authorization.split(' ')[1])
        console.log('user!!!', data)
        if(data.valid !== 1) return next(new Errors.UnAuth("Invalid token"));
